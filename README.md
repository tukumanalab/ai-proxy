...
ダッシュボードの「Settings」からNGワードを追加・編集・削除できます。

初回起動時は`.env`ファイルの設定が自動的にデータベースに移行されます：
...
Gemini APIを使用することで、単純なキーワードマッチでは検知できない以下のケースも検出できます：
...
1. **高速な一次チェック**: キーワードマッチング
2. **LLMチェック**: キーワードマッチで検出されなかった場合のみ実行

APIキーが設定されていない場合は、従来のキーワードマッチのみで動作します。
